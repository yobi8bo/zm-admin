<template>
  <div class="page">
    <a-card :bordered="false">
      <div class="toolbar">
        <a-button type="primary" @click="handleCreate"><plus-outlined />新增部门</a-button>
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'action'">
            <a-space size="small">
              <a-button type="link" size="small" @click="handleCreate(record.id)">新增子部门</a-button>
...
<script setup>
  ...
  const formRef = ref()
  const form = reactive({ id: null, parent_id: undefined, name: '', leader: '', phone: '', email: '', sort: 0, status: 1 })
  const formRules = { name: [{ required: true, message: '请输入部门名称' }] }

  async function loadData() {
    loading.value = true
    try {
      tableData.value = await deptApi.list() || []
    } finally {
      loading.value = false
    }
  }
  ...
  function handleCreate(parentId = undefined) {
    Object.assign(form, { id: null, parent_id: parentId, name: '', leader: '', phone: '', email: '', sort: 0, status: 1 })
  ...
  async function handleSubmit() {
    ...
    try {
      form.id ? await deptApi.update(form.id, form) : await deptApi.create(form)
